...
1. 客户端请求建立 TLS 连接
   ...
   > 客户端发送支持的 TLS 版本、Chiper Suite(对称加密、非对称加密、hash算法)、随机数
...
3. 客户端验证服务器证书
   ...
   > 
   >
   > 证书机构的签发方（签发方公钥直接在操作系统里）

4. 客户端信任服务器后，和服务器协商（非对称加密）对称密钥

5. 使用对称密钥开始通信

   ![HTTPS连接](https://dingyx.oss-cn-shenzhen.aliyuncs.com/picgo/HTTPS连接.png)
